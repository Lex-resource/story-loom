...
export default function useGenerationActions({ project, editor, generation, services, setters }) {
  ...
  const { editedOutline, editedContent, editedTitle } = editor;
  const { continueChaptersCount, continueWordCount } = generation;
  const { addLog, showToast, loadProjects, loadProjectStatus, loadChapters, loadChapterDetails } = services;
  const {
  ...
  const handleProjectSelect = async (project) => {
    ...
    // The project effect handles the initial status and chapter refresh.
  };
  ...
  const handleCreateProject = async (e) => {
    ...
    try {
      ...
      const freshData = await loadProjects();
  ...
  const handleStartGenerate = async (batchSize = 1, wordCount = 3000) => {
    ...
    try {
      ...
      addLog(
        '系统',
        `已成功启动续写任务（续写 ${batchSize} 章，单章目标 ${wordCount > 0 ? wordCount + ' 字' : '无限制'}）。准备拉起大纲策划与创作...`,
        'system',
      );
  ...
  const handleRewriteChapter = async (prompt = '') => {
    if (!activeProject || !activeChapter) return false;
    setAutoFollowGeneration(true);
    setActionLoading(true);
    try {
      const rewritePrompt = prompt.trim();
      await pipelineApi.rewrite(activeProject.id, {
        use_existing_outline: true,
        custom_prompt: rewritePrompt || null,
  ...
  const handleRewriteOutline = async (prompt = '') => {
    if (!activeProject) return false;
    const instruction = prompt.trim();
    if (!instruction) {
      showToast('请填写重新大纲的要求', 'warning');
      return false;
    }

    setActionLoading(true);
    try {
      const currentOutline = editedOutline ? JSON.parse(editedOutline) : activeProject.outline || {};
      const data = await outlineApi.rewrite(activeProject.id, currentOutline, instruction);
      if (data.outline) {
        setEditedOutline(JSON.stringify(data.outline, null, 2));
      }
      addLog('系统', '全书骨架大纲已根据当前大纲和用户要求重新规划。', 'system');
      return true;
    } catch (e) {
      if (e instanceof SyntaxError) {
        showToast(`当前大纲 JSON 格式有误: ${e.message}`, 'warning');
      } else {
        showToast(`重新大纲失败: ${e.message}`);
      }
      return false;
    } finally {
      setActionLoading(false);
