      ...
      <div className="projects-grid">
        {loading ? (
            ...
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="project-card project-card--loading" aria-hidden="true">
                <div>
                  ...
                  <div className="skeleton" style={{ height: '6px', width: '100%', borderRadius: '4px', marginBottom: '8px' }} />
          ...
          <article key={p.id} className="project-card">
            <button className="project-card__open" onClick={() => onSelectProject(p)} aria-label={`打开作品：${p.title}`}>
              ...
              <div className="project-card__stats">
                <span>{p.current_chapter || 0} / {target} 章</span>
                <span>{p.total_chars?.toLocaleString() || 0} 字</span>
              </div>
            </button>
            <button className="icon-btn project-card__delete" onClick={(e) => handleDeleteProject(p.id, e)} title="删除作品" aria-label={`删除作品：${p.title}`}>
              <Trash2 size={15} />
            </button>
          </article>
        )})}
