...
import { Activity, ListTodo, Pause, Play, RefreshCw, Search, Eye, X } from 'lucide-react';
...
export default function ConsoleLogs({
  ...
  // 全局队列徽标:订阅共享轮询(useWorkerStatus),失败静默隐藏徽标。
  const { status: queue } = useWorkerStatus();
    ...
    <div className="floating-console">
      <div className="pane-header console-header">
        <div className="console-title">
          ...
          {queue && (queue.pending_jobs > 0 || queue.running_jobs > 0) && (
            <span
              title={`全局队列:排队 ${queue.pending_jobs} / 运行 ${queue.running_jobs}${queue.claim_paused ? '(已暂停领取)' : ''}`}
              style={{
                fontSize: '11px',
                marginLeft: '12px',
                display: 'flex',
                ...
                color: queue.claim_paused ? 'var(--vermilion)' : 'var(--text-secondary)',
              }}
            >
              <ListTodo size={12} />
              队列 {queue.pending_jobs} · 运行 {queue.running_jobs}
              {queue.claim_paused ? ' · 已暂停领取' : ''}
            </span>
          )}
        </div>
        ...
        <div className="console-actions">
          <button
            className="btn btn-secondary console-action"
            onClick={handlePause}
            title="暂停生成"
            aria-label="暂停生成"
          >
          ...
          <button
            className="btn btn-danger console-action"
            onClick={handleRewriteChapter}
            disabled={actionLoading}
            title={rewriteLabel}
            aria-label={rewriteLabel}
          >
            <RefreshCw size={14} /> <span>{rewriteLabel}</span>
          </button>
        </div>
      </div>
