...
export function BasicOutlineTab({obj, isShortOutline, creativeContract, getField, setPreservedField, setEditedOutline, updateOutlineField}) {
  return (
    <div className="outline-section-card animate-fadeIn">
      <div className="outline-grid-2x3">
        <div className="form-group-outline">
          ...
          <input className="form-input-outline" value={getField(obj, ['书名', '标题'])} onChange={(e) => {
            const next = { ...obj };
        ...
        <div className="form-group-outline">
          ...
          <input className="form-input-outline" value={obj["类型"] || ''} onChange={(e) => updateOutlineField('类型', e.target.value)} placeholder={isShortOutline ? '如：知乎短篇、悬疑、甜虐...' : '如：仙侠、玄幻...'} />
        </div>
        <div className="form-group-outline">
          <label className="form-label-outline">{isShortOutline ? '故事基调 (Tone)' : '风格 (Style)'}</label>
          <input className="form-input-outline" value={getField(obj, ['风格', '故事基调'])} onChange={(e) => {
            const next = { ...obj };
        ...
        <div className="form-group-outline">
          ...
          <input className="form-input-outline" type="number" value={getField(obj, ['总章数', '总章节数'])} onChange={(e) => {
            const next = { ...obj };
      ...
      <div className="form-group-outline">
        <label className="form-label-outline">{isShortOutline ? '一句话钩子/简介 (Hook)' : '结局走向 (Ending Direction)'}</label>
        <textarea
      ...
      {!isShortOutline && (
        <div style={{ marginTop: '18px' }}>
          <div className="outline-card-header" style={{ marginBottom: '10px' }}>
            ...
            <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>自动优化不会改写这些作品承诺</span>
          </div>
          <div className="outline-grid-2x3">
            {[
              ['核心卖点', '读者为什么持续追读这本书'],
  ...
  return (
    <div className="outline-section-card animate-fadeIn">
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {stages.map((arc, idx) => (
          <div key={idx} className="outline-card-inner glow-shadow outline-stage-card" style={{ borderLeft: '4px solid var(--vermilion)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div>
                <label className="form-label-outline" style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>幕名/卷名</label>
                <input className="form-input-outline" style={{ fontSize: '13px', padding: '6px 10px' }} value={arc["卷名"] || arc["幕名"] || ''} onChange={(e) => {
                  const newStages = [...stages];
              ...
              <div>
                <label className="form-label-outline" style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>章节范围</label>
                <input className="form-input-outline" style={{ fontSize: '13px', padding: '6px 10px' }} value={arc["章节范围"] || ''} onChange={(e) => {
                  const newStages = [...stages];
            ...
            <div style={{ marginBottom: '8px' }}>
              <label className="form-label-outline" style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>{stagesKey === '卷纲' ? '卷目标' : isShortOutline ? '主要推进' : '概要'}</label>
              <textarea className="form-textarea-outline" style={{ minHeight: '70px', resize: 'vertical', fontSize: '13px', padding: '8px 10px', lineHeight: '1.5' }} value={arc["卷目标"] || arc["概要"] || arc["progress"] || ''} onChange={(e) => {
                const newStages = [...stages];
                const fieldKey = stagesKey === '卷纲' ? "卷目标" : arc["progress"] !== undefined ? "progress" : "概要";
                newStages[idx][fieldKey] = e.target.value;
                updateOutlineField(stagesKey, newStages);
              }} placeholder={isShortOutline ? '这一阶段的钩子、信息增量和反转推进...' : '本卷的起承转合核心剧情...'} />
            </div>
            <div>
              <label className="form-label-outline" style={{ fontSize: '11px', color: 'var(--vermilion)' }}>{stagesKey === '卷纲' ? '主要对手与冲突' : '核心冲突'}</label>
              <textarea className="form-textarea-outline" style={{ minHeight: '70px', resize: 'vertical', fontSize: '13px', padding: '8px 10px', lineHeight: '1.5' }} value={arc["主要对手"] || arc["核心冲突"] || ''} onChange={(e) => {
                const newStages = [...stages];
                newStages[idx][stagesKey === '卷纲' ? "主要对手" : "核心冲突"] = e.target.value;
                updateOutlineField(stagesKey, newStages);
              }} placeholder={isShortOutline ? '本阶段的主要冲突、爽点、反转或情绪爆点...' : '本阶段的主要矛盾与高潮对抗...'} />
            </div>
            {stagesKey === '卷纲' && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '10px', marginTop: '10px' }}>
                {[
                  ...
                  <div key={field}>
                    <label className="form-label-outline" style={{ fontSize: '11px' }}>{field}</label>
                    <textarea className="form-textarea-outline" style={{ minHeight: '64px', fontSize: '12px' }} value={arc[field] || ''} onChange={(e) => {
                      const newStages = [...stages];
        ...
        <button
          ...
          style={{ alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: '4px', padding: '6px 12px', fontSize: '12px' }}
          onClick={() => {
            const newStages = [...stages];
            newStages.push(isShortOutline
              ? { "幕名": "", "章节范围": "", "progress": "", "核心冲突": "" }
              : stagesKey === '卷纲'
                ? { "卷名": "", "章节范围": "", "卷目标": "", "主要对手": "", "升级变化": "", "阶段兑现": "", "失败代价": "", "核心事件": [] }
                : { "幕名": "", "章节范围": "", "概要": "", "核心冲突": "" });
            updateOutlineField(stagesKey, newStages);
          }}
        >
  ...
  return (
    <div className="outline-section-card animate-fadeIn">
          ...
          return (
            <div key={idx} className="character-card-premium">
              ...
              <textarea
                ...
                style={{ minHeight: '60px', resize: 'vertical', fontSize: '12px', padding: '8px 10px', lineHeight: '1.5' }}
      ...
      <button
        type="button"
        className="btn btn-secondary"
        style={{ alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: '4px', padding: '6px 12px', fontSize: '12px', marginTop: '8px' }}
        onClick={() => {
          const newChars = [...characters];
  ...
  return (
    <div className="outline-section-card animate-fadeIn">
        ...
        {foreshadowings.map((fb, idx) => (
          <div key={idx} className="foreshadowing-card-premium">
            ...
            <textarea
              ...
              style={{ minHeight: '70px', resize: 'vertical', fontSize: '12px', padding: '8px 10px', lineHeight: '1.5' }}
      ...
      <button
        ...
        onClick={() => {
          const newFb = [...foreshadowings];
          newFb.push(isShortOutline ? { "伏笔名称": "", chapter: 1, resolved_chapter: 1, description: "" } : { "埋设章节": 0, "收回章节": 0, "描述": "" });
          updateOutlineField(foreshadowingKey, newFb);
        }}
      >
        <Plus size={14} /> 添加伏笔
