...
export default function WorkspaceValidator({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', overflowY: 'auto', flexGrow: 1, minHeight: 0 }}>
      <div className="glass-panel" style={{ padding: '20px', borderRadius: '8px' }}>
        <h3 style={{ marginTop: 0 }} className="text-gradient">天道法则自动审计</h3>
        <p className="text-secondary text-sm">此审查对比了当前章节正文与活文档中已编译的设定（如林枫境界、死伤状态、法术规则等）。如有逻辑严重偏离，将阻止流程自动进入下一章。</p>
        {isValidatorStreaming && (
          <div className="warning-badge warning pulsing-border" style={{ marginTop: '12px', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
      ...
      {validationResult ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {validationResult.errors?.map((err, idx) => (
            <div key={idx} style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', padding: '12px', borderRadius: '8px', background: 'rgba(244, 63, 94, 0.1)', border: '1px solid rgba(244, 63, 94, 0.3)' }}>
          ...
          {validationResult.warnings?.map((warn, idx) => (
            <div key={idx} style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', padding: '12px', borderRadius: '8px', background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.3)' }}>
          ...
          {validationResult.infos?.map((info, idx) => (
            <div key={idx} style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', padding: '12px', borderRadius: '8px', background: 'rgba(59, 130, 246, 0.1)', border: '1px solid rgba(59, 130, 246, 0.3)' }}>
          ...
          {storyIssues.map((issue, idx) => (
            <div key={`story-${idx}`} style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', padding: '12px', borderRadius: '8px', background: 'rgba(168, 85, 247, 0.08)', border: '1px solid rgba(168, 85, 247, 0.28)' }}>
              <Sparkles style={{ color: '#a855f7', flexShrink: 0 }} />
              <div>
                <span className="font-semibold text-sm">【故事与文笔优化提示】 {issue.category ? issue.category : 'story'}</span>
          ...
          {!hasVisibleIssues && !validationResult.streaming && (
            <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary)' }}>
              ...
              <p className="mt-2 font-semibold" style={{ color: 'var(--text-primary)' }}>未发现逻辑冲突与设定矛盾</p>
          ...
          {validationResult.errors?.length > 0 && (
            <div className="glass-panel" style={{ padding: '16px', borderRadius: '8px', border: '1px dashed var(--color-red, #ef4444)', background: 'rgba(244, 63, 94, 0.05)', display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '10px' }}>
              <span style={{ fontWeight: 'bold', color: 'var(--color-red, #ef4444)', display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                ...
                <button className="btn btn-secondary" onClick={handleRewriteChapter} style={{ fontSize: '13px', padding: '6px 12px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <RefreshCw size={14} /> AI 重新写本章
                </button>
                <button className="btn btn-danger" onClick={() => {
                  if (confirm('确认强制发布放行吗？这会忽略本章的逻辑报错，强制将该章标记为已发布并继续后面的生成。')) {
