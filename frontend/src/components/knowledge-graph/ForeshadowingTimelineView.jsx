...
export default function ForeshadowingTimelineView({ projectId }) {
  ...
  useEffect(() => {
    ...
    knowledgeGraphApi
      .foreshadowingTimeline(projectId, { signal: request.signal })
      .then((json) => {
        if (request.isCurrent()) setData(json);
      })
      .catch((err) => {
        if (request.isCurrent() && err.name !== 'AbortError') setData({ chains: [] });
      })
      .finally(() => {
    ...
    <div className="kg-view">
      <header className="kg-view-header">
        <div>
          ...
          <p className="kg-view-desc">追踪各主线伏笔在各章节的埋设、增强与回收生命周期。</p>
        </div>
        <GraphToolbar searchValue={search} onSearchChange={setSearch} searchPlaceholder="搜索伏笔线索…" />
      </header>
                  ...
                  <div className="foreshadow-timeline-line">
                    {chain.events?.map((ev, idx) => (
                      <div key={idx} className="foreshadow-timeline-event">
                        ...
                        <div className={`event-type-dot ${ev.type}`}>{EVENT_TYPE_LABEL[ev.type] || ev.type}</div>
