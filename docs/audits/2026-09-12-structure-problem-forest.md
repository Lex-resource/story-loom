...
| P3 | A3/E2/G1/D2 | 模板散布 / 角色域大文件 / 孤儿类 / 静默吞错 |
...
## 终版处置(2026-09-14):全部遗留问题清点完毕

对剩余三项组织类问题逐一评估后**有意维持现状**,理由如下(评估=处置):

| 编号 | 评估结论 |
|---|---|
| G2 prompt_hints.py 832 行 | 主体是 v7–v43 **版本化 hint(冻结生产表面,A28/V43)**——永不改动的代码,拆分只给冻结面加导入间接层;其余为同类字符串构建器,内聚于"提示词提示"单一关注点。维持现状。 |
| E2 角色域大文件 | 胖路由已删(J1a);`character_card_service`(528 行)按角色卡领域内聚,`routers/characters.py`(443 行/8 路由)规模正常。维持现状。 |
...
- **K2 ✅** 冗余延迟导入清理(顶部导入补 branch_default_graph)
